	...
	<table>
        ...
        <tbody>
            <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                <td class="checkTD"><input type="checkbox" class="check" name="depInfo" value="<?php echo ($val["dept_id"]); ?>"></td>
            	<td class="num"><?php echo ($val["dept_id"]); ?></td>
                <td class="name">
                    <?php echo (str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;',$val["dept_level"])); echo ($val["dept_name"]); ?>				</td>
                <td class="process"><?php echo ($val["dept_pid"]); ?></td>
                <td class="node"><?php echo ($val["dept_sort"]); ?></td>
                <td class="time"><?php echo ($val["dept_remark"]); ?></td>
                <td class="operate">
                    <a href="<?php echo U('Del','id='.$val[dept_id]);?>">删除</a>
                    <a href="<?php echo U('Edit','id='.$val[dept_id]);?>">修改</a>
                </td>
...
<script type="text/javascript">
    ...
    $(".del").on("click", function () {
        ...
        location.href="/Admin/Dept/Del/id/"+str;
    })
    //复选框全选，反选
